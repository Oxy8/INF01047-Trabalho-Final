...
// Vértices de um cubo unitário (o skybox será escalonado por matriz)
...
// -------------------------------------------------------------
// Cria o cubemap e configura o VAO/VBO do cubo
// -------------------------------------------------------------
unsigned int LoadSkybox(const std::vector<std::string>& faces)
{
    ...
    for (unsigned int i = 0; i < faces.size(); i++)
    {
        ...
        {
            std::cerr << "Falha ao carregar textura do skybox: " << faces[i] << std::endl;
            stbi_image_free(data);
        }
    }
...
// -------------------------------------------------------------
// Desenha o skybox (chamado após desenhar a cena)
// -------------------------------------------------------------
void DrawSkybox(Shader& shader, const glm::mat4& view, const glm::mat4& projection)
{
    ...
    shader.use();
    shader.setInt("object_id", SKYBOX);
    shader.setInt("TextureCubemapSkybox", 0);
